...
*/
int
Pipe (struct Commands commands[], int number)
{
...
int
Recognizer (char *input)
{
  char *pch, *pch2, *pch3;
  char *programOutput[CHAR_MAX], *pipeDirection[CHAR_MAX], *temp[CHAR_MAX],
    *process[CHAR_MAX], *parameter[CHAR_MAX];
  ;
...
//BloklarD1na ayrD1lan girdinin pipe ve yC6nlendirme iC'erme durumunun kontrolC<
  ...
  for (int i = 0; i < a; i++)
	...
	{
	  //AlD1nan pointer dizinin < iEaretine gC6re bC6lC<nmesi ve her bC6lC<nen bloDun bir pointer ile iEaretlenmesi
	  ...
	  int k = 0;
	  //AlD1nan pointer dizinin | iEaretine gC6re bC6lC<nmesive her bC6lC<nen bloDun bir pointer ile iEaretlenmesi
	  ...
	  for (int l = 0; l < k; l++)
	    {
	      //AlD1nan pointer dizinin boEluklarD1na  bC6lC<nmesi, her bC6lC<nen bloDun bir pointer ile iEaretlenmesi ve sistem komutu olup olmadD1DD1nD1n belirlenmesi.Sistem komutu deDil ise isminin dC<zenlenmesi
	      pch2 = strtok (temp[l], " ");
	      int m = 0, counter = 0;
	      while (pch2 != NULL)
		{
      ...
      if (directionbool[i] == 0 && pipebool[i] == 0)
	{
	  ...
	  while (pch2 != NULL)
	      ...
	      if (m == 0)
		{
		  if (*(pch2) == '.' && *(pch2 + 1) == '/')
		    {
		      commands[0].type = 0;
		      commands[0].prosses[0] = pch2 + 2;

		    }
		  else
		    {
		      commands[0].type = 1;
		      commands[0].prosses[0] = pch2;

		    }
		}
	      else if (m == 1)
		{
		  commands[0].prosses[1] = pch2;
		  commands[0].prosses[2] = NULL;
	  ...
	  if (m == 1)
	    {
	      commands[0].prosses[1] = NULL;

	    }
	  counter++;
	}
      /////////////////////////////////////////////////SADECE PIPE ICERENLER/////////////////////////////////////////
  ...
  if (getcwd (cwd, sizeof (cwd)) != NULL)
    {
      ...
      printf ("\033[1;36m%s@%s:\033[1;35m~%s\033[0;33m>", username, hostname,
	      cwd);
